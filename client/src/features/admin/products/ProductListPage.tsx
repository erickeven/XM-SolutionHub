...
export function ProductListPage() {
  ...
  const restoreMutation = useMutation({
    mutationFn: (id: string) => updateProduct(id, { status: 'DRAFT' }),
    onSuccess: () => {
      message.success('已恢复');
      queryClient.invalidateQueries({ queryKey: ['admin-products'] });
    },
  ...
  const handleMoveToRecycle = (record: AdminProductListItem) => {
    Modal.confirm({
      title: '移入回收站',
      content: `确定将产品 ${record.model} 移入回收站？移入后可在回收站筛选查看。`,
      okText: '移入回收站',
      okType: 'danger',
      cancelText: '取消',
  ...
  const handleRestore = (record: AdminProductListItem) => {
    Modal.confirm({
      title: '恢复产品',
      content: `确定恢复产品 ${record.model}？恢复后将进入草稿状态。`,
      okText: '恢复',
      okType: 'primary',
      cancelText: '取消',
